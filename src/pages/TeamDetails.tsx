        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="border border-f1-border">
              ...
              <div className="p-6 md:p-8">
                ...
                {/* Title */}
                <h1 className="font-display text-4xl md:text-5xl font-extrabold uppercase tracking-tight text-white leading-none mb-2">
                  {team.shortName}
                </h1>
                <p className="text-sm text-f1-muted mb-6">{team.name}</p>
                ...
                {/* Description */}
                <p className="text-sm text-f1-muted max-w-2xl leading-relaxed mb-8">
                  {team.description}
                </p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-f1-border">
                  {stats.map(({ label, value, icon: Icon, accent }) => (
                    <div key={label} className="bg-f1-card p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <Icon size={14} className={accent} />
                        <span
                          className={`font-mono text-[10px] uppercase tracking-[0.12em] ${accent}`}
                        >
                          {label}
                        </span>
                      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
                ...
                {team.drivers.map((driverId, index) => {
                  ...
                  return (
                    <Link key={driverId} to={`/driver/${driver.id}`}>
                      <div className="bg-f1-card group hover:bg-f1-border/40 transition-colors p-5 flex items-center justify-between">
                        <div className="flex items-center gap-5">
                          {/* Car number */}
                          <span
                            className="font-display text-3xl font-extrabold leading-none w-10 text-right"
                            style={{ color: team.color }}
                          >
                            {driver.number}
                          </span>
                          <div>
                            <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-f1-muted mb-0.5">
                              Driver {String(index + 1).padStart(2, "0")}
                            </p>
                            <p className="font-display text-xl font-extrabold uppercase text-white group-hover:text-f1-accent transition-colors">
                              {driver.name}
                            </p>
                          </div>
                        </div>
            ...
            <div>
                ...
                <div className="p-5">
                  <div className="flex flex-col gap-0">
                    {[
                      ...
                      >
                        <span className="text-[11px] text-f1-muted shrink-0">
                          {label}
                        </span>
                        <span className="font-mono text-[12px] text-white text-right leading-snug">
                          {value}
                        </span>
                      </div>
                    ))}
                  ...
                  {team.championships > 0 && (
                    <div className="mt-4 pt-4 border-t border-f1-border/30">
                      <div className="flex items-center gap-2">
                        <Trophy size={13} className="text-f1-gold" />
                        <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-f1-gold">
              ...
              {/* Season progress — wins/podiums bar */}
              <div className="mt-4 border border-f1-border p-5">
                <div className="h-1 bg-f1-border mb-4" />
